...
router.post(
  ...
  async (req, res) => {
    ...
    if (!sysSettings.aiFeatures?.chat) {
      return res.status(503).json({
        error: "O chat está temporariamente desactivado. Tente mais tarde."
      });
    }
...
router.post(
  ...
  async (req, res) => {
    const { getSettings } = require('../services/systemSettingsService');
    const sysSettings = await getSettings();
    if (!sysSettings.aiFeatures?.imageAnalysis) {
      return res.status(503).json({ error: "Análise de imagem desactivada." });
    }
